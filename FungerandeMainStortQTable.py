                ...
                for y1 in range(-int(GRIDHEIGHT)+1, int(GRIDHEIGHT)):
                    for x2 in range(-int(GRIDWIDTH) + 1, int(GRIDWIDTH)):
                        for y2 in range(-int(GRIDHEIGHT) + 1, int(GRIDHEIGHT)):
                            q_table[(x1,y1),(x2,y2)] = [np.random.uniform(-5, 0) for i in range(NUMBER_OF_ACTIONS)]
        else:
    ...
    def qLearning(self):
        ...
        for episode in range(HM_EPISODES):
            ...
            for i in range(200):
                ...
                obs = (self.distanceTo(player,food), self.distanceTo(player,self.closestRaycast()))
                ...
                if player.hitWall:
                    ...
                    reward = FOOD_REWARD
                elif previousPosition == player.pos:
                    reward = -MOVE_PENALTY * 2
                else:
                    reward = -MOVE_PENALTY

                new_obs = (player-food, player-self.closestRaycast())
